...
async def test_delete_own_tweet(client: AsyncClient, test_user_1: User):
    ...
    delete_resp = await client.delete(f"/api/tweets/{tweet_id}", headers={"api-key": str(test_user_1.api_key)})
    assert delete_resp.status_code == 200
    assert delete_resp.json()["result"] is True


@pytest.mark.anyio
async def test_delete_other_users_tweet(session: AsyncSession, client: AsyncClient, test_user_1: User, test_user_2: User):
    # user_1 creates tweet
    await create_tweet(session=session, request=CreateTweetRequest(tweet_data="Protected", tweet_media_ids=[]), author_id=test_user_1.id)
    result = await session.execute(select(Tweet.id).where(Tweet.author_id == test_user_1.id))
    tweet_id = result.scalar()

    # user_1 tries to delete it
    resp = await client.delete(f"/api/tweets/{tweet_id}", headers={"api-key": str(test_user_2.api_key)})
...
async def test_get_feed_empty(client: AsyncClient, test_user_1: User):
    resp = await client.get("/api/tweets", headers={"api-key": str(test_user_1.api_key)})
    assert resp.status_code == 200
    data = resp.json()
...
@pytest.mark.anyio
async def test_get_feed_sorted_by_popularity(session: AsyncSession, client: AsyncClient, test_user_1: User, test_user_2: User):

    # subscribe user_1 on user_2
    await follow_user(session, follower_id=test_user_1.id, following_id=test_user_2.id)

    # 2 tweets by user_2
    tweet_1 = await create_tweet(session, CreateTweetRequest(tweet_data="Popular", tweet_media_ids=[]), test_user_2.id)
    tweet_2 = await create_tweet(session, CreateTweetRequest(tweet_data="Less popular", tweet_media_ids=[]), test_user_2.id)
    ...
    # get_feed for user_1
    response = await client.get("/api/tweets", headers={"api-key": str(test_user_1.api_key)})
    assert response.status_code == 200
